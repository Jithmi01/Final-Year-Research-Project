...
class _ARCurrencyDetectorPageState extends State<ARCurrencyDetectorPage> {
  ...
  int _cooldownSeconds = 5; // Wait 5 seconds after detection
  ...
  void _startContinuousScanning() {
    ...
    // Scan every 1 second, but respect cooldown period
    _scanTimer = Timer.periodic(Duration(seconds: 1), (timer) {
      if (_isScanning && !_waitingForNextScan) {
        ...
        if (remainingSeconds > 0) {
          setState(() {
            _statusMessage = "Next scan in $remainingSeconds seconds...";
          });
        } else {
          // Cooldown finished, ready to scan again
          setState(() {
            _waitingForNextScan = false;
            _statusMessage = "Scanning for currency...";
          });
        ...
        if (mounted) {
          setState(() {
            ...
            if (_detectedCurrencies.isEmpty) {
              ...
              // Continue scanning immediately if nothing detected
            } else {
              // Currency detected! Start cooldown period
  ...
  void _manualScan() {
    // Force a scan even during cooldown
    setState(() {
      _waitingForNextScan = false;
